...
class VerifyViewCtrl: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        print("\(UserDefaults.standard.value(forKey: "Otp") as? String)")
    ...
    @IBAction func verifyButtonClicked(_ sender: Any) {
        if((verificationCodeTextField.text?.characters.count) == 5){
            if termsAndCondationSwitch.isOn{
               if(verificationCodeTextField.text == UserDefaults.standard.value(forKey: "Otp") as? String ){
                    UserDefaults.standard.set("2", forKey: Key.KEY_USER_IS_LOGIN)
                    let myVC = storyboard?.instantiateViewController(withIdentifier: "moreItemViewCtrlID") as! moreItemViewCtrl
                    let transition = CATransition()
                    transition.duration = 0.5
                    transition.timingFunction = CAMediaTimingFunction(name: kCAMediaTimingFunctionEaseInEaseOut)
                    ...
                    let alert = UIAlertController(title: "", message: "OTP Not Match", preferredStyle: UIAlertControllerStyle.alert)
                    ...
                    DispatchQueue.main.async {
                        ...
                        if let parseJSON = json {
                            ...
                            if status == "true"{
                                ...
                                let otp = parseJSON["otp"] as AnyObject
                                print("\(otp)")
