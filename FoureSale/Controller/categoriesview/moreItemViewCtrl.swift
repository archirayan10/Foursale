...
class moreItemViewCtrl: UIViewController, UITableViewDelegate, UITableViewDataSource,UIPopoverPresentationControllerDelegate {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        {
            if(UserDefaults.standard.value(forKey: Key.KEY_USER_IS_LOGIN) as? String == "2")
            {
                return sectionTwoArray.count
            }
            else if(UserDefaults.standard.value(forKey: Key.KEY_USER_IS_LOGIN) as? String == "1"){
                return 1
            }
            else{
                return 1
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(indexPath.section == 1)
        {
            if(UserDefaults.standard.value(forKey: Key.KEY_USER_IS_LOGIN) as? String == "2")
            {
                ...
                mycell.itemNameLabel.text="Verify"
                mycell.itemImage.image = #imageLiteral(resourceName: "checkmark")
                mycell.countryLabel.text = " "
            }
            else{
